...
const provide = {
  singleton: () => (target: any) =>
    fluentProvide(target).inSingletonScope().done()(target),

  transient: () => (target: any) =>
    fluentProvide(target).inTransientScope().done()(target),
};
...
function bindToContainer(identifier: any): void {
...
const lazyInject = (identifier: any) => (
  target: any,
  key: string,
  descriptor?: any,
) => {
